...
install_requirements() {
    ...
    sudo yum update -y && sudo yum install -y docker nginx docker-compose || {
...
required_restarts() {
    ...
    sudo chown "$USER"  /var/lib/docker  || {
...
deploy() {
    ...
    docker build -t notes-app /home/ec2-user/django-notes-app/ && /usr/local/bin/docker-compose -f ./django-notes-app/docker-compose.yml up -d || {
